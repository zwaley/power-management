...
- 命名：端口名称一律使用原始 port_name 字段；任何场景不得重命名或翻译。
- 文案：未连接/断开态/N/A 统一呈现为“空闲”。
- 布局：设备居中；输入左、输出右；双向端口按序号均分左右；对端端口与A端端口同Y对齐。
- 边样式：设备→端口为灰色无箭头；端口↔端口为高亮实线带箭头；支持平行边避让。
- Tooltip：端口类型(type)、规格(spec)、方向(direction)、连接到、对端端口、电缆型号/备注（如有）。
- API 契约：GET /graph_data/{device_id}?level=port&layout=bus&show_all_ports=0|1。
