...
class PortTopologyService:
    ...
    def _is_valid_port_name(self, name):
        # 修复None值处理：如果name是None，直接返回False，避免创建虚假的remote节点
        if name is None:
            return False
        if not name:
            return False
    ...
    def _process_port(self, port_info, i, side, device, nodes, edges, port_count, base_index=0):
        ...
        # 修复连接创建条件：只要有对端设备且不是自连接就创建连接，不强制要求电缆信息
        is_different_device = str(device_id) != str(remote_device_id)
        should_create_remote = (
            is_different_device and
